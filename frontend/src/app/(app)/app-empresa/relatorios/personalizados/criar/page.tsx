...
export default function CriarRelatorioPersonalizadoPage() {
  ...
  const onSubmit = async (data: RelatorioFormData) => {
    try {
      ...
      await api.post('/relatorios/personalizados', payload)
      
      toast({
        title: 'Relatório criado',
        description: 'O relatório personalizado foi criado com sucesso.'
      })

      router.push('/app-empresa/relatorios/personalizados')
    } catch (error: any) {
      toast({
        title: 'Erro ao criar',
        description: error.response?.data?.error || 'Não foi possível criar o relatório.',
        variant: 'destructive'
      })
    } finally {
      setSaving(false)
  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-y-auto bg-background">
        <Header title="Novo Relatório Personalizado" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input
                        id="nome"
                        {...register('nome')}
                        placeholder="Ex: Relatório de Produtividade Mensal"
                        error={!!errors.nome}
                      />
                      {errors.nome && (

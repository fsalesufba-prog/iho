...
export default function CentrosCustoPage() {
  ...
  const carregarStats = async () => {
    try {
      const response = await api.get('/centros-custo/stats', {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/centros-custo/${selectedCentro.id}`)
      toast({
        title: 'Sucesso',
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Gerencie fornecedores, prestadores de serviços e avaliações
            </p>
          </div>
        </div>

        <Link href="/app-empresa/centros-custo/novo">
          <Button>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{stats.mediaGeral.toFixed(1)}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.avaliacoes}</p>
              </div>
                ...
                {loading ? (
                  ...
                  centros.map((centro) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/app-empresa/centros-custo/${centro.id}`)}
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <span className={`text-lg font-bold ${getNotaColor(centro.avaliacoes.media)}`}>
                            {centro.avaliacoes.media.toFixed(1)}
                          </span>
                          <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                        </div>
                      </TableCell>
                      <TableCell>{formatDate(centro.avaliacoes.ultima)}</TableCell>
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              onClick={() => router.push(`/app-empresa/centros-custo/${centro.id}/edit`)}
                            >
                              <Edit className="h-4 w-4 mr-2" />
                              Editar
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() => router.push(`/app-empresa/centros-custo/${centro.id}/avaliacoes/nova`)}
                            >
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir o centro de custo "{selectedCentro?.nome}"?
